...
def get_optimizer(model, config: edict):
    name = config.optimizer.name
    params = config.optimizer.params
    if name == "Adam":
        optimizer = Adam(model.parameters(), **params)
    elif name == "SGD":
        optimizer = SGD(model.parameters(), **params)  # type: ignore
    elif name == "diffRGrad":
        optimizer = diffRGrad(model.parameters(), **params)  # type: ignore
    else:
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if self.version == 0 or self.version == 1:
                    ...
                    dfc = 9. / (1. + torch.exp(-diff)
                                ) - 4  # DFC2 = 9/(1+e-(.5/g/)-4 #range .5,5
                ...
                if N_sma >= 5:
                    ...
                    p_data_fp32.addcdiv_(-step_size * group['lr'], exp_avg1,
                                         denom)
